...
int main()
{
    printf("spi flash program1\n");
    //BOOTREG = INSTR_RAM_BASE_ADDR;
    //memctl_init();
    //read flash ID
    spi_setup_master(1);
    *(volatile int*)(SPI_REG_CLKDIV) = 0x16;
    //spi_setup_dummy(0, 0);
    //int id = 0;
    ...
    while((spi_get_status() & 0xFFFF) != 1);
    spi_setup_cmd_addr(CMD_RDCR, 8, 0, 0);             //rdcr 
    spi_set_datalen(8);
    spi_start_transaction(SPI_CMD_RD, SPI_CSN0);
    spi_read_fifo(&ver, 8);
    printf("CR1: %x \n", ver);
/*
    ...
    while((spi_get_status() & 0xFFFF) != 1);
    spi_setup_cmd_addr(WRR, 8, 0x00040000, 16);                  //all write CMD don't have dummy cycles, B7 is set 32 bit addr
    spi_set_datalen(0);
    spi_start_transaction(SPI_CMD_WR, SPI_CSN0);
    printf("WRR \n");
...
#define BRRD          0x16
    ...
    while((spi_get_status() & 0xFFFF) != 1);
    spi_setup_cmd_addr(BRWR, 8, 0x80000000, 8);          //set 32 bit addr
    spi_set_datalen(0);
    spi_start_transaction(SPI_CMD_WR, SPI_CSN0);
    printf("BRWR \n");

    while((spi_get_status() & 0xFFFF) != 1);
    spi_setup_cmd_addr(BRRD, 8, 0, 0);                  
    spi_set_datalen(8);
    spi_start_transaction(SPI_CMD_RD, SPI_CSN0);
    spi_read_fifo(&ver, 8);
    if(ver & 0x80)
      printf("Get in 32 bit mode\n");
    else
    {
      printf("Fail to get in 32 bit mode\n");
      while(1);
    }
  /*
    int da_test[64];
    ...
    flash_write(0x1000, da_test, 64);
    flash_read(0x1000, da_read, 64);
    for(int i=0;i<64;i++)
    {
      if(da_test[i] != da_read[i])
        {
          printf("%x_%x\n",da_test[i],da_read[i]);
          break;
        }
...
*/
   ...
   */
    ...
    data_r = (int *) 0x23000000;
    int page_num = total_size_r / 64;
    int remaind_num = total_size_r % 64;
    flash_addr = header[0];
    for(int i = 0; i < page_num ; i++)
    {
       flash_write(flash_addr, data_r, 64);
       flash_read(flash_addr, read_check, 64);
       for(int j = 0; j < 64; j++)
       { 
...
/*#ifdef 0
    int move_num;
    int *move_ptr = (int*)Ann_Config;
    move_num = IM_DEPTH0 + WEIGTH_DEPTH0 * 2 + BIAS_DEPTH0 / 4 + 128;
    page_num = move_num / 64;
    remaind_num = move_num % 64;
    flash_addr = 0x00050000;
    printf("moving config array number: %d ..\n", move_num);
    for(int i = 0; i < page_num ; i++)
    {
       flash_write(flash_addr, move_ptr, 64);
       flash_read(flash_addr, read_check, 64);
       for(int j = 0; j < 64; j++)
       { 
          printf("data_addr:%x, flash_addr:%x, data:%x \n",move_ptr, flash_addr, read_check[j] );
        if(*(move_ptr + j) != read_check[j])
         {
           printf("write check error..At:%x \n", (int)(move_ptr + j));
           printf("Write Data:%x Read Data:%x \n", *(move_ptr + j),  read_check[j]);
           return 0;
         }
       }
       move_ptr += 64;
    ...
    flash_write(flash_addr, move_ptr, remaind_num);
    flash_read(flash_addr, read_check, remaind_num);
       for(int j = 0; j < remaind_num; j++)
       { 
        printf("data_addr:%x, flash_addr:%x, data:%x \n",move_ptr, flash_addr, read_check[j] );
    ...
    printf("config array is write proper! \n");
#endif*/
  //-----------------------------------------------------------
  // Read header
  //-----------------------------------------------------------
    ...
    int *instr = (int *) 0x24100000;
    int instr_size =  header_ptr[2];
    int *data = (int *)  0x24200000;
...
#ifdef STORE_DATA
    addr = 0x00050000;
    move_ptr = (int*)Ann_Config;
    data = (int *) 0x24300000;
    block_num = move_num / 1024;
    num = move_num % 1024;
    for (int i = 0; i < block_num; i++)  //reads 4KB blocks
    {
        spi_setup_cmd_addr(0x13, 8, addr, 32);
