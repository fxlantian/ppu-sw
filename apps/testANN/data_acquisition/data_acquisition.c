...
extern int dis_line;
void send_image(unsigned char* image_sampled)
{
    int label=0;
    int k=0;
    if((*((volatile int*)UART_REG_LSR) & 0x1) == 0x1){
      label = *(volatile int*)UART_REG_RBR;
    label = label - 48;
      //output image
      for(int i=0;i<28;i++)
      {
...
void sample_image(volatile unsigned char* image_addr, unsigned char* image_sampled)
{
    ...
    unsigned char* image_addr_ver_ll;
    unsigned char* image_addr_ver_nl;
    image_addr = image_addr + 640*239 - 60 * 640;
    for(i=0;i<28;i++)
    {
      image_addr_ver = 8 * (i + 26) + image_addr;
      for(j=0;j<28;j++)
      {
        image_addr_ver -= 640 * 4; //pick a pixel every 8 line
        //image_addr_ver_ll = image_addr_ver - 640;
        image_addr_ver_nl = image_addr_ver + 640;
        k=(*image_addr_ver + *(image_addr_ver - 2));
        //k+=(*image_addr_ver_ll + *(image_addr_ver_ll - 2) + *(image_addr_ver_ll + 2));
        k+=(*image_addr_ver_nl + *(image_addr_ver_nl - 2));
        image_sampled[i*28 + j] = k/4;
      }
    }
}

void disp_samp_image(unsigned char* image_sampled)
{
    ...
    //int x_hpol,x_lpol,y_hpol,y_lpol;
    int center_x=0;
    int center_y=0;
    int center_n=0;
    for(i=0;i<28;i++)
    ...
    for(i=0;i<28;i++)
    {
      for(j=0;j<28;j++)
      {
        image_sampled[ver] = 255 - ((image_sampled[ver] - min) * 255) / max_dif;
        if(image_sampled[ver] < 90)
            image_sampled[ver] = 0;
        if(image_sampled[ver] > 110){
          center_x+=j;
          center_y+=i;
          center_n++;
        }
    ...
    center_x = center_x / center_n;
    center_y = center_y / center_n;

    int center_x_dif = center_x - 14;
    int center_y_dif = center_y - 14;
    LCD_Show2Num(350,300,center_x,2);
    LCD_Show2Num(390,300,center_y,2);
    //printf("center_x_dif:%d\n",center_x_dif);
    //printf("center_y_dif:%d\n",center_y_dif);
    if(center_x_dif > 0){
        for (i = 0; i < 28 - center_x_dif; i++)
        {
          for (j = 0; j < 28; j++)
          {
            image_sampled[i + j*28] = image_sampled[i + center_x_dif + j*28];
          }
        }
        for (i = 28 - center_x_dif; i < 28; i++)
        {
          for (j = 0; j < 28; j++)
          {
            image_sampled[i + j*28] = 0;
          }
        }
      }
      else if(center_x_dif < 0){
        for (i = 27; i > -center_x_dif; i--)
        {
          for (j = 0; j < 28; j++)
          {
            image_sampled[i + j*28] = image_sampled[i + center_x_dif + j*28];
      ...
      if(center_y_dif > 0){
        for (i = 0; i < 28 - center_y_dif; i++)
        {
          for (j = 0; j < 28; j++)
          {
            image_sampled[i*28 + j] = image_sampled[(i + center_y_dif)*28 + j];
          }
        }
        for (i = 28 - center_y_dif; i < 28; i++)
        {
          for (j = 0; j < 28; j++)
          {
            image_sampled[i*28 + j] = 0;
          }
        }
      }
      else if(center_y_dif < 0){
        for (i = 27; i > -center_y_dif; i--)
        {
          for (j = 0; j < 28; j++)
          {
            image_sampled[i*28 + j] = image_sampled[(i + center_y_dif)*28 + j];
...
int main() {
 ...
 // }
    LCD_Clear(BLACK);
    LCD_DrawRectangle(349, 99, 406, 156);
    POINT_COLOR = RED;
    LCD_ShowString(330 ,40 ,"ICT PPU Demo");
    POINT_COLOR = WHITE;
    LCD_ShowString(330 ,60 ,"Enable Edge AI");
       ...
       if(Isr_Type != 0)
        {
           ...
           if(Isr_Type == 1) {
                ...
                disp_samp_image(image_sampled);
                send_image(image_sampled);
