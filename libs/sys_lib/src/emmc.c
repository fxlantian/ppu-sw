...
//current_task_status  *p =&current_task;
 Card_info card_info[2];
 Card_info *the_card_info = (Card_info*)&card_info;
...
u32 emmc_init_controller()
{
	...
	emmc_set_bits(EMMC_REG_CTRL, CTRL_RESET); //reset controller
	emmc_undo_idma_settings();
	emmc_set_register(EMMC_REG_BLKSIZ, 0x0200);
	emmc_set_register(EMMC_REG_BYTCNT, 0x0200);
	plat_delay(0xffff);
    /*Init clock set*/
    emmc_set_register(EMMC_REG_UHS_REG_EXT,0xC1020000);   //ext_clk_div 8, clk_2x = 400Mhz, clk_in = 50Mhz, clk_drv and clk_sample delay 2 clk_2x, 
	...
	//emmc_clear_bits(EMMC_REG_CTYPE,buffer_reg);     // 1_bit  mode  
...
//	clock_val = (1 << the_ip_status.num_of_cards) - 1 ;	
    ...
    emmc_set_register(EMMC_REG_CLKDIV, SD_ONE_BIT_BUS_FREQ_DIV);  //set clk_div 63, fout = 400k 2*clk_divider0
    ...
    the_ip_status.present_cdetect = 1; //no have connect pin with wifi
	...
	printf("FIFO_DEPTH read from Hardware = %d\n",fifo_thresh);
	the_ip_status.fifo_depth = fifo_thresh;
	printf("FIFO depth = %d\n", the_ip_status.fifo_depth);
	fifo_thresh = 64;
	the_ip_status.fifo_threshold = fifo_thresh;
	/* Tx Watermark */
	emmc_clear_bits(EMMC_REG_FIFOTH, 0xfff);
	emmc_set_bits(EMMC_REG_FIFOTH, fifo_thresh);  //as soon as there is an item available, a dma/fifo requst will set
	/* Rx Watermark */
	emmc_clear_bits(EMMC_REG_FIFOTH, 0x0fff0000);
	emmc_set_bits(EMMC_REG_FIFOTH, (fifo_thresh-1) << 16); //if fifo has receive as many as data, a dma/fifo request will set
	
	/* Enumerate the cards connected */
	emmc_enumerate_card_stack(num_of_cards);

	/* use dma */
	the_ip_status.idmac_enabled = 1;
	emmc_setup_desc_list(1, RINGMODE);
	emmc_idma_settings();
	emmc_dump_registers();
...
u32 emmc_enumerate_the_card(u32 slot_num)
{
	...
	*/
    ...
    the_card_info[slot_num].divider_val = SD_ONE_BIT_BUS_FREQ_DIV;
...
Card_Type emmc_get_card_type(u32 slot)
{
	...
	//printf("buffer_reg, EMMC_REG_CDETECT:%x\n", buffer_reg);
	//CARD_DETECT line not connect, since we have a sdio device
...
void ISR_EMMC () 
{
	...
	if (TSK_STAT_ABSENT != current_task.cmd_status) {
		current_task.postproc_callback(&current_task, (u32 *)EMMC_REG_RINTSTS);
		...
		if(current_task.idma_mode_on ==1){
		    emmc_handle_standard_idsts(&current_task , L_EMMC_REG_IDSTS);
		}
		if (current_task.error_status) {
			interrupt_already_done = 1;
			//Command done has already came and error is set. So clean up the interrupt status
			if(current_task.cmd_status == TSK_COMMAND_DONE){ 
				//FIFO is flushed to avoid RXDR and TXDR Interrupts in Slave mode of operation
				emmc_reset_fifo(); 
				// goto FINISH;  //Wait Till Command Done Interrupt
				//printf("RINTSTS now is : %08x\n",L_EMMC_REG_RINTSTS);
				emmc_set_register(EMMC_REG_RINTSTS, 0xFFFFFFFF);
				emmc_set_register(EMMC_REG_IDSTS,  0xFFFFFFFF);
			}
		}
		...
		*/
		...
		printf("TSK_STAT_ABSENT but interrupts getting generated!!!\n");
		emmc_reset_fifo(); //FIFO is flushed to avoid RXDR and TXDR Interrupts in Slave mode of operation
		emmc_set_register(EMMC_REG_RINTSTS, 0xFFFFFFFF);			
		emmc_set_register(EMMC_REG_IDSTS, 0xFFFFFFFF); // Clear IDMAC interrupts
	}
			 ...
			 u32 flags)
{
	...
	printf("Sending CMD:0x%08x ARG:0x%08x\n", cmd_register, arg_register);
	if(cmd_register==0x04d)
		emmc_dump_registers();
...
u32 emmc_set_sd_high_speed(u32 slot)
{
        ...
        resp_buffer = (u32 *)malloc(sizeof(u32)*4);      //plat_alloc  change to  plat_alloc  ??? ly
        the_data_buffer = (u8 *)malloc(sizeof(u32)*16); 
			      ...
			      emmc_postproc_callback custom_post)
{
	...
	/*This hack is required as CMD42 block size is not equal to write_block_size*/
	if(CMD42 == (custom_command & 0x3f)){
	...
	printf("start = %x, end = %x, card_size = %d, block_size = %d\n",
	     start, end, the_card_info[slot].card_size, the_block_size);

	/*check start and end address is aligned to card block address*/
	if (!(custom_command & CUSTCOM_STREAM_RW)) {
		if ((start % the_block_size)
		    || (end % the_block_size)) {
			printf("misalign start and end addr.\n");
	 ...
	 */
	if ((!the_term_function) && (start == end)) {   //if start == end and not send cmd12 ,return ERR
		return ERRADDRESSRANGE;
	}


	if (!(CUSTCOM_DONT_TRANS & custom_command)) {
		printf("GO TO TRANS STATE\n");
		/* Check if the card is in standby state and put in trans
		   state.
		 */
		if ((retval = emmc_put_in_trans_state(slot))) {
    ...
    the_card_info[slot].divider_val = SD_FPP_FREQ_DIV;
	/* Set card the block len */
	//since we use a sdhc card, cmd16 is useless.
	if ((!(custom_command & CUSTCOM_DONT_CMD16))
	    && (!(custom_command & CUSTCOM_STREAM_RW))) {
		printf("Sending CMD16 TO set blocklen\n");
		if ((retval =
		     emmc_send_serial_command(slot, CMD16,      //CMD16 : SET_BLOCKLEN command
						  the_block_size,
						  resp_buffer,
						  NULL, 0, NULL, NULL))) {
	...
	if (read_or_write) { //write 
        ...
        printf("the_ip_status.idmac_enabled:%d\n",the_ip_status.idmac_enabled);
	...
	if(the_ip_status.idmac_enabled == 1){
		...
		/*As we use it in raw system*/
		//dma_addr = plat_map_single((current_task.bus_device), data_buffer, dma_len, BIDIRECTIONAL);
		dma_addr1 = (dma_addr_t)data_buffer;
		printf("dma_addr1:%x\n", dma_addr1);
	...
	if(the_ip_status.idmac_enabled == 1){
		current_task.idma_mode_on = 1;
	}
	printf("current_task->cmd_status:%d\n",current_task.cmd_status);
...
if((command_to_send == CMD25) || (command_to_send == CMD18))
{
	...
	if ((custom_command & CUSTCOM_DONTSTDBY)) {   
		printf("Not to go to standby state after the data transfer");
		if (read_or_write) {
...
u32 emmc_reset_sd_card(u32 slot)
{
	...
	u32 buffer_reg;
	u32 retval = 0;
	u32 clock_freq_to_set = SD_FOD_DIVIDER_VALUE;
...
u32 emmc_set_sd_2_0_voltage_range(u32 slot)
{
	...
	while (count) {
       ...
       //ACMD41 :send host capacity support info and ask card seed its OCR reg value 
		printf("Sending ACMD41 to slot %x\n", slot);
		retval = emmc_send_serial_command(slot, ACMD41, new_ocr, &resp_buffer, NULL, 0, NULL, NULL);
		printf("GOT OCR AS = 0x%08x\n", resp_buffer);
		if ((resp_buffer & OCR_POWER_UP_STATUS) && (!retval)) {
...
//	printf("Entered %s\n", __FUNCTION__);
	...
	the_card_info[slot].divider_val = SD_ONE_BIT_BUS_FREQ_DIV;
	/* Reset the card. Since we really dont know as to from where the call has been made*/
	printf("Sending CMD0 to slot %x\n", slot);
	if ((retval = emmc_send_serial_command(slot, CMD0, 0, NULL, NULL, 0, NULL, NULL))) {
		printf("Sending CMD0 to slot error %x\n", slot);
...
u32 emmc_set_sd_ddr(u32 slot)
{
        ...
        resp_buffer = (u32 *)malloc(sizeof(u32)*4);         //how to replace    lly
        the_data_buffer = (u8 *)malloc(sizeof(u32)* 16);
			  ...
			  u32 the_interrupt_status)
{
	...
	u32 task_bytes_left, last_word_check = 0, count;
	// u32 aligned_address; 
...
u32 emmc_is_desc_free(DmaDesc *desc)
{
	if((desc->desc0 & DescOwnByDma) == 0)
		return 0; //if OWN bit is not set Descriptor is free
	else
		return 1; //if OWN bit is still set Desccriptor is owned by DMA		
}
...
//	emmc_set_register(EMMC_REG_DBADDR,(u32)virt_to_phys(desc));
	emmc_set_register(EMMC_REG_DBADDR,(u32)desc);
...
s32 emmc_setup_desc_list(u32 no_of_desc, u32 desc_mode)
{
	...
	current_task.desc_mode        = RINGMODE;
	    ...
	    if(first_desc==0)
	    {
	    	first_desc = 0x21000000;
	    }
	    printf("ALLOC ADDR FOR first_desc:%08x\n",(u32)first_desc);
	    if(first_desc == NULL){
    	...
    	for(i =0; i <(no_of_desc-1); i++){
    		...
    		printf("ALLOC ADDR FOR second_desc:%08x\n",(u32)second_desc);
    		if(second_desc == NULL){	
	    		printf("Error in Descriptor Memory allocation in Chain mode\n");
		    	return -1;
		    }
    		first_desc->desc3             = (u32)second_desc;
...
void emmc_undo_idma_settings(void){
		...
		emmc_set_register(EMMC_REG_INTMSK,emmc_get_slave_intmask_task_status()); //Get back the slave mode iterrupt mask
		return;
	}

void emmc_idma_settings(void){
		printf(" Setting dma..\n");
		current_task.idma_mode_on = 0;
		emmc_set_register(EMMC_REG_IDINTEN,0xffffffff); //enable all interrupt
		emmc_set_bits(EMMC_REG_CTRL, CTRL_USE_IDMAC); //Idmac Enable
		emmc_set_bits(EMMC_REG_BMOD, BMOD_FB); //Fixed Burst Length
		emmc_set_bits(EMMC_REG_BMOD, BMOD_DE); //Idmac Enable
		emmc_set_bits(EMMC_REG_BMOD, BMOD_DSL_TWO); //Idmac Enable
		emmc_clear_bits(EMMC_REG_INTMSK, INTMSK_TXDR|INTMSK_RXDR); //Blocking rx/tx fifo interrupts
		emmc_set_register(EMMC_REG_PLDMND,0x01);//TODO:WHY DMA SUSPEND
		return;
	}
...
{
	printf("LOCKING --plat_alloc_consistent_dmaable_memory---\n");
...
u32 emmc_get_status_of_card(u32 slot, Card_state * status)
{
	...
	printf("send CMD13 to get status\n");
	if ((retval =
   ...
   if((SD_MEM_2_0_TYPE == the_card_info[slot].card_type )|| (SD_MEM_3_0_TYPE == the_card_info[slot].card_type)) {
	...
	retval = emmc_read_write_bytes_MMC_4_2(slot,resp_buff,buffer,
		...
		retval =  emmc_read_write_bytes(slot, resp_buff, buffer,
					      ...
					      0, NULL, NULL,
					      read_or_write_to_send,
					      sect_size << CUSTOM_BLKSIZE_SHIFT|CUSTCOM_DONTSTDBY, //10<<12=0xa000
					      NULL,
					      NULL);
#if 1
	{
...
void dma_ownbit_set(void){
	current_task.desc_next->desc0 |=  DescOwnByDma;
}