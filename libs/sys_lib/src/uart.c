...
//  set_pin_function(1, FUNC_UART);
  ...
  *(volatile unsigned int*)(UART1_REG_FCR) = 0xA7; //enables 16byte FIFO and clear FIFOs
  *(volatile unsigned int*)(UART1_REG_LCR) = 0x03; //sets 8N1 and set DLAB to 0
  //*(volatile unsigned int*)(UART1_REG_LCR) |= 0x40; 
  *(volatile unsigned int*)(UART1_REG_IER) = ((*(volatile unsigned int*)(UART1_REG_IER)) & 0xF0) | 0x02; // set IER (interrupt enable register) on UART
}
...
void uart1_send(const unsigned char* str, unsigned int len) {
  ...
  while(len > 0) {
    ...
    for(i = 0; (i < UART_FIFO_DEPTH) && (len > 0); i++) {
      // wait until there is space in the fifo
      while( (*(volatile unsigned int*)(UART1_REG_LSR) & 0x20) == 0);
      // load FIFO
      *(volatile unsigned int*)(UART1_REG_THR) = *str++;
...
char uart1_getchar() {
  ...
  return *(volatile int*)UART1_REG_RBR;
}
...
void uart1_sendchar(const unsigned char c) {
  ...
  // load FIFO
  *(volatile unsigned int*)(UART1_REG_THR) = c;
}

void uart1_wait_tx_done(void) {
  // wait until there is space in the fifo
  while( (*(volatile unsigned int*)(UART1_REG_LSR) & 0x40) == 0);
}